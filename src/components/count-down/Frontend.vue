            ...
            <h4>
             ...
             <section class="date">
                 <p v-if="!bEnd">
                                    ...
                                    <strong style="font-size:1.24rem;"><template v-if="type == 0">后开始</template><template v-else>后结束</template></strong>
                                
                </p>
                <p style="font-size:1.3rem;" v-else>
                    敬请期待
                </p>
                </section> 
...
export default {
    ...
    created() {
        this.$http.post(this.url(`console&c=${window.apiAuthority}&a=seckill`), qs.stringify({
            num: this.nNumber
        })).then(({
            data: {
                seckill: {
            ...
            type == 0 ? this.endTime = begin_time : this.endTime = end_time
            if (goods) this.spikeProlist = goods
            this.type = type
        })

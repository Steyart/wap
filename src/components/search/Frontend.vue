<template>
    <div class='search' :style="{position:oPosition.position,width:preview ? '318px' : ''}">
        ...
        <div class="navigation" v-show="navInfo">
            <nav>
                <li  v-for="(item,$index) in arr" @click="toggle($index)" :class="{active:$index==active}" :key="$index">
...
export default {
    ...
    mounted() {
        ...
        let _this = this
        $.ajax({
            url:'/mobile/index.php?m=site&c=index&a=categories',
            type:'POST',
            dataType:'json',
            data:{
                token: '',
...
.navigation li.active{
    color: #000;
    border-bottom: 1px solid #1eb57d
}

    ...
    <div class='pages' :class="{'close-page-list':!bPageListTitle}">
        ...
        <ec-dialog :title="nUpdateId==-1 ? '新增页面' : '修改页面'" size="tiny" v-model="bAddPageDialog" :modal-append-to-body="false" @close="closePageDialog">
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <ec-button size="small" type="primary" @click="confirmAddPage('btnAddPage')">确 定</ec-button>
            </div>
        </ec-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        updateDialog(o) {
            ...
            let picUrl = ''
            if (o.thumb_pic != '') {
                picUrl = `${window.PC_URL}data/gallery_album/original_img/${
                    o.thumb_pic
                }`
            }
        ...
        addOrUpdatePage(page) {
            ...
            if (this.nUpdateId == -1) {
                ...
                if (this.uploadData.topicId) {
                    Message.success('已为您自动创建关联PC端的专题页！')
                } else {
    ...
    computed: {
        ...
        uploadData() {
            ...
            if (oUrl.query.topic_id) {
                ...
                this.sTitle = oUrl.query.title || '自动创建专题'
            }
            return {
                title: this.sTitle.trims(),
                ruid: 0,
